...
;(function (global, factory) {
    ...
    ***************************************************************************** */
		...
		HashtagMatch.prototype.getAnchorHref = function () {
			...
			switch (serviceName) {
				...
				default: // Shouldn't happen because Autolinker's constructor should block any invalid values, but just in case.
					throw new Error(
						'Unknown service name to point hashtag to: ' + serviceName,
					)
			}
		}
		/**
	...
	var ipStr =
		'(?:[' + decimalNumbersStr + ']{1,3}\\.){3}[' + decimalNumbersStr + ']{1,3}'
	...
	var EmailMatcher = /** @class */ (function (_super) {
		...
		function EmailMatcher() {
			 ...
			 */
			_this.localPartCharRegex = new RegExp(
				'[' + alphaNumericAndMarksCharsStr + "!#$%&'*+/=?^_`{|}~-]",
			)
			/**
		...
		EmailMatcher.prototype.parseMatches = function (text) {
				...
				if (prevChar === ':') {
					...
					if (localPartCharRegex.test(char)) {
						...
						currentEmailMatch = new CurrentEmailMatch(
							__assign({}, currentEmailMatch, { hasMailtoPrefix: true }),
						)
					} else {
			...
			function stateDomainDot(char) {
				if (char === '.' || char === '-') {
					...
					currentEmailMatch = new CurrentEmailMatch(
						__assign({}, currentEmailMatch, { hasDomainDot: true }),
					)
				} else {
					// Anything else
					captureMatchIfValidAndReset()
	...
	var UrlMatchValidator = /** @class */ (function () {
		 ...
		 */
		UrlMatchValidator.urlMatchDoesNotHaveProtocolOrDot = function (
			urlMatch,
			protocolUrlMatch,
		) {
			return (
		 ...
		 */
		UrlMatchValidator.urlMatchDoesNotHaveAtLeastOneWordChar = function (
			urlMatch,
			protocolUrlMatch,
		) {
	...
	var UrlMatcher = /** @class */ (function (_super) {
		...
		function UrlMatcher(cfg) {
			 ...
			 */
			_this.wordCharRegExp = new RegExp(
				'[' + alphaNumericAndMarksCharsStr + ']',
			)
			_this.stripPrefix = cfg.stripPrefix
			_this.stripTrailingSlash = cfg.stripTrailingSlash
		...
		UrlMatcher.prototype.parseMatches = function (text) {
			...
			var _loop_1 = function () {
				...
				if (
					offset > 0 &&
					protocolRelativeMatch &&
					this_1.wordCharRegExp.test(prevChar)
				) {
					return 'continue'
				}
				...
				var foundCommonScheme = ['http://', 'https://'].find(function (
					commonScheme,
				) {
		 ...
		 */
		UrlMatcher.prototype.matchHasInvalidCharAfterTld = function (
			urlMatch,
			schemeUrlMatch,
		) {
			if (!urlMatch) {
	...
	var HashtagMatcher = /** @class */ (function (_super) {
		...
		function HashtagMatcher(cfg) {
			 ...
			 */
			_this.nonWordCharRegex = new RegExp(
				'[^' + alphaNumericAndMarksCharsStr + ']',
			)
			_this.serviceName = cfg.serviceName
			return _this
	...
	var PhoneMatcher = /** @class */ (function (_super) {
		...
		PhoneMatcher.prototype.parseMatches = function (text) {
			...
			while ((match = matcherRegex.exec(text)) !== null) {
				...
				if (
					this.testMatch(match[3]) &&
					this.testMatch(matchedText) &&
					contextClear
				) {
					matches.push(
						new PhoneMatch({
	...
	function parseHtml(html, _a) {
		...
		function stateTagOpen(char) {
			if (char === '!') {
				...
				currentTag = new CurrentTag(
					__assign({}, currentTag, { isClosing: true }),
				)
			} else if (char === '<') {
				...
				state = 3 /* TagName */
				currentTag = new CurrentTag(
					__assign({}, currentTag, { isOpening: true }),
				)
			} else {
				// Any other
				state = 0 /* Data */
		...
		function stateTagName(char) {
			if (whitespaceRe.test(char)) {
				currentTag = new CurrentTag(
					__assign({}, currentTag, { name: captureTagName() }),
				)
		...
		function stateMarkupDeclarationOpen(char) {
			if (html.substr(charIdx, 2) === '--') {
				...
				currentTag = new CurrentTag(
					__assign({}, currentTag, { type: 'comment' }),
				)
				state = 14 /* CommentStart */
			} else if (html.substr(charIdx, 7).toUpperCase() === 'DOCTYPE') {
				charIdx += 7 // "consume" characters
				currentTag = new CurrentTag(
					__assign({}, currentTag, { type: 'doctype' }),
				)
				state = 20 /* Doctype */
			} else {
	...
	var Autolinker = /** @class */ (function () {
		...
		function Autolinker(cfg) {
			...
			this.newWindow =
				typeof cfg.newWindow === 'boolean' ? cfg.newWindow : this.newWindow
			...
			parseHtml(textOrHtml, {
				...
				onText: function (text, offset) {
					...
					if (skipTagsStackCount === 0) {
						...
						var htmlCharacterEntitiesRegex =
							/(&nbsp;|&#160;|&lt;|&#60;|&gt;|&#62;|&quot;|&#34;|&#39;)/gi
						...
						textSplit.forEach(function (splitText, i) {
							...
							if (i % 2 === 0) {
								var textNodeMatches = _this.parseText(
									splitText,
									currentOffset_1,
								)
