...
function track(obj, propertyNames) {
	...
	propertyNames.forEach(function (propertyName) {
		...
		if (
			propertyName === OBSERVABLES_PROPERTY ||
			propertyName === SUBSCRIBABLE_PROPERTY
		) {
			return
		}
		...
		var origValue = obj[propertyName],
			...
			observable = ko.isObservable(origValue)
				? origValue
				: isArray
				? ko.observableArray(origValue)
...
function getObservable(obj, propertyName) {
	...
	return (
		(allObservablesForObject && allObservablesForObject[propertyName]) || null
	)
}
