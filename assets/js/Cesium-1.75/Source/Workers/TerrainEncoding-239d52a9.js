...
define([
		...
		cameraPosition: {
			...
			set: function (cameraPosition) {
				...
				var vhMagnitudeSquared =
					Cartesian2.Cartesian3.magnitudeSquared(cv) - 1.0
	...
	var scratchEllipsoidShrunk = Cartesian2.Ellipsoid.clone(
	...
	EllipsoidalOccluder.prototype.computeHorizonCullingPointFromRectangle =
		function (rectangle, ellipsoid, result) {
			...
			if (
				Cartesian2.Cartesian3.magnitude(bs.center) <
	...
	function getPossiblyShrunkEllipsoid(ellipsoid, minimumHeight, result) {
		if (
			...
			ellipsoid = Cartesian2.Ellipsoid.fromCartesian3(
	...
	function computeMagnitude(ellipsoid, position, scaledSpaceDirectionToPoint) {
		...
		var magnitudeSquared =
			Cartesian2.Cartesian3.magnitudeSquared(scaledSpacePosition)
		...
		var cosAlpha = Cartesian2.Cartesian3.dot(
			direction,
			scaledSpaceDirectionToPoint,
		)
	...
	function computeScaledSpaceDirectionToPoint(ellipsoid, directionToPoint) {
		if (
			Cartesian2.Cartesian3.equals(directionToPoint, Cartesian2.Cartesian3.ZERO)
		) {
		...
		return Cartesian2.Cartesian3.normalize(
			directionToPointScratch,
			directionToPointScratch,
		)
	}
	...
	function TerrainEncoding(
		...
		if (
			...
			var maxDim = Math.max(
				Cartesian2.Cartesian3.maximumComponent(dimensions),
				hDim,
			)
			...
			toENU = Transforms.Matrix4.inverseTransformation(
				fromENU,
			...
			Transforms.Matrix4.setTranslation(
				matrix,
			...
			var translationMatrix = Transforms.Matrix4.fromTranslation(
				minimum,
				matrix4Scratch,
			)
			var scaleMatrix = Transforms.Matrix4.fromScale(
				dimensions,
				matrix4Scratch2,
			)
			var st = Transforms.Matrix4.multiply(
				translationMatrix,
				scaleMatrix,
				matrix4Scratch,
			)
	...
	TerrainEncoding.prototype.encode = function (
		...
		if (this.quantization === TerrainQuantization$1.BITS12) {
			...
			var h = _Math.CesiumMath.clamp(
				(height - this.minimumHeight) / hDim,
				0.0,
				1.0,
			)

			Cartesian2.Cartesian2.fromElements(
				position.x,
				position.y,
				cartesian2Scratch,
			)
	...
	TerrainEncoding.prototype.decodePosition = function (buffer, index, result) {
		...
		if (this.quantization === TerrainQuantization$1.BITS12) {
			...
			return Transforms.Matrix4.multiplyByPoint(
				this.fromScaledENU,
				result,
				result,
			)
	...
	TerrainEncoding.prototype.decodeTextureCoordinates = function (
		...
		return Cartesian2.Cartesian2.fromElements(
			buffer[index + 4],
			buffer[index + 5],
			result,
		)
