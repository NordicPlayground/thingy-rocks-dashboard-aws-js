...
define([
	...
	'./AttributeCompression-f8ced8b6',
	'./EncodedCartesian3-46a75b7e',
	'./IndexDatatype-7948b6db',
	...
	function barycentricCoordinates(point, p0, p1, p2, result) {
		...
		if (!when.defined(p0.z)) {
			if (
				Cartesian2.Cartesian2.equalsEpsilon(
					point,
					p0,
	...
	GeometryPipeline.reorderForPreVertexCache = function (geometry) {
		...
		if (when.defined(indices)) {
			...
			var indicesOut = IndexDatatype.IndexDatatype.createTypedArray(
				numVertices,
				numIndices,
			)
	...
	GeometryPipeline.projectTo2D = function (
		...
		projection = when.defined(projection)
			? projection
		...
		for (var i = 0; i < values3D.length; i += 3) {
			var value = Cartesian2.Cartesian3.fromArray(
				values3D,
				i,
	...
	GeometryPipeline.computeNormal = function (geometry) {
		...
		for (i = 0; i < numIndices; i += 3) {
			...
			normalsPerTriangle[j] = Cartesian2.Cartesian3.cross(
				v1,
				v2,
	...
	GeometryPipeline.computeTangentAndBitangent = function (geometry) {
		...
		//>>includeStart('debug', pragmas.debug);
		if (
			!when.defined(attributes.position) ||
			!when.defined(attributes.position.values)
		) {
			throw new Check.DeveloperError(
				'geometry.attributes.position.values is required.',
			)
		...
		if (
			!when.defined(attributes.normal) ||
			!when.defined(attributes.normal.values)
		) {
			throw new Check.DeveloperError(
		...
		for (i = 0; i < numVertices; i++) {
			...
			Cartesian2.Cartesian3.normalize(
				Cartesian2.Cartesian3.subtract(t, normalScale, t),
				t,
			)
	...
	GeometryPipeline.compressVertices = function (geometry) {
		...
		if (when.defined(extrudeAttribute)) {
			...
			for (i = 0; i < numVertices; ++i) {
				...
				if (
					Cartesian2.Cartesian3.equals(toEncode1, Cartesian2.Cartesian3.ZERO)
				) {
					i2 += 2
					continue
				}
	...
	var interpolateAndPackBoolean = function (
		...
		currentValues[insertedIndex] =
			v1 + v2 + v3 > _Math.CesiumMath.EPSILON6 ? 1 : 0
	}
	...
	function computeTriangleAttributes(
		...
		if (when.defined(extrudeDirections)) {
			var d0 = Cartesian2.Cartesian3.fromArray(
				extrudeDirections,
				i0 * 3,
				p0Scratch,
			)
			var d1 = Cartesian2.Cartesian3.fromArray(
				extrudeDirections,
				i1 * 3,
				p1Scratch,
			)
			var d2 = Cartesian2.Cartesian3.fromArray(
				extrudeDirections,
				i2 * 3,
				p2Scratch,
			)
	...
	function splitLongitudeTriangles(instance) {
		...
		var tangents = when.defined(attributes.tangent)
			...
			: undefined
		var texCoords = when.defined(attributes.st)
	...
	function computeLineAttributes(
		...
		var p0 = Cartesian2.Cartesian3.fromArray(positions, i0 * 3, p0Scratch)
		if (
			Cartesian2.Cartesian3.equalsEpsilon(p0, point, _Math.CesiumMath.EPSILON10)
		) {
	...
	function updateAdjacencyAfterSplit(geometry) {
		...
		for (var j = 0; j < length; j += 3) {
			...
			var prevPosition = Cartesian2.Cartesian3.unpack(
				prevPositions,
				j,
				cartesian3Scratch2,
			)
			...
			var nextPosition = Cartesian2.Cartesian3.unpack(
				nextPositions,
				j,
				cartesian3Scratch3,
			)
	...
	function splitLongitudePolyline(instance) {
		...
		var texCoords = when.defined(attributes.st)
			? attributes.st.values
			: undefined
		var colors = when.defined(attributes.color)
		...
		for (i = 0; i < length; i += 4) {
			...
			var p0 = Cartesian2.Cartesian3.fromArray(
				positions,
				i0 * 3,
				cartesian3Scratch0,
			)
			...
			if (when.defined(intersection)) {
				...
				if (when.defined(colors)) {
					var c0 = Transforms.Cartesian4.fromArray(
						colors,
						i0 * 4,
						cartesian4Scratch0,
					)
					var c2 = Transforms.Cartesian4.fromArray(
						colors,
						i2 * 4,
						cartesian4Scratch0,
					)
				...
				if (when.defined(texCoords)) {
					var s0 = Cartesian2.Cartesian2.fromArray(
						texCoords,
						i0 * 2,
						cartesian2Scratch0,
					)
