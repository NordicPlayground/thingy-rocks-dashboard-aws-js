...
define([
	...
	var scratchPlane = new Plane.Plane(
		new Cartesian2.Cartesian3(1.0, 0.0, 0.0),
		0.0,
	)
	...
	OrthographicOffCenterFrustum.prototype.computeCullingVolume = function (
		...
		plane.w = -Cartesian2.Cartesian3.dot(
			Cartesian2.Cartesian3.negate(right, negateScratch),
			point,
		)
		...
		plane.z = -up.z
		plane.w = -Cartesian2.Cartesian3.dot(
			Cartesian2.Cartesian3.negate(up, negateScratch),
			point,
		)
	...
	OrthographicOffCenterFrustum.prototype.getPixelDimensions = function (
		...
		//>>includeStart('debug', pragmas.debug);
		if (
			!when.defined(drawingBufferWidth) ||
			!when.defined(drawingBufferHeight)
		) {
			throw new Check.DeveloperError(
	...
	function update$2(frustum) {
		...
		if (
			...
			frustum._perspectiveMatrix =
				Transforms.Matrix4.computePerspectiveOffCenter(
					l,
					r,
					b,
					t,
			...
			frustum._infinitePerspective =
				Transforms.Matrix4.computeInfinitePerspectiveOffCenter(
					l,
					r,
					b,
					t,
	...
	FrustumGeometry.unpack = function (array, startingIndex, result) {
		...
		var origin = Cartesian2.Cartesian3.unpack(
			array,
			startingIndex,
			scratchPackorigin,
		)
	...
	FrustumGeometry._computeNearFarPlanes = function (
		...
		var view = Transforms.Matrix4.computeView(
			origin,
			z,
			y,
			x,
		...
		if (frustumType === PERSPECTIVE) {
			...
			inverseView = Transforms.Matrix4.inverseTransformation(
				view,
				scratchInverseMatrix,
			)
		}
