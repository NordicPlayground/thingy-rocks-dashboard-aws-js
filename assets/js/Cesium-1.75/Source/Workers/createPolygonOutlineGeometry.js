...
define([
	...
	'./EllipsoidTangentPlane-2bf0d02b',
	'./ArcType-13a53523',
	'./EllipsoidRhumbLine-6133712f',
	'./PolygonPipeline-2140d99c',
	'./PolygonGeometryLibrary-dd9c1d5b',
], function (
	when,
	Check,
	_Math,
	...
	function createGeometryFromPositions(
		...
		if (!perPositionHeight) {
			...
			if (arcType === ArcType.ArcType.GEODESIC) {
				for (i = 0; i < length; i++) {
					numVertices +=
						PolygonGeometryLibrary.PolygonGeometryLibrary.subdivideLineCount(
							positions[i],
				...
				for (i = 0; i < length; i++) {
					numVertices +=
						PolygonGeometryLibrary.PolygonGeometryLibrary.subdivideRhumbLineCount(
							ellipsoid,
							positions[i],
							positions[(i + 1) % length],
							minDistance,
			...
			for (i = 0; i < length; i++) {
				...
				if (arcType === ArcType.ArcType.GEODESIC) {
					tempPositions =
						PolygonGeometryLibrary.PolygonGeometryLibrary.subdivideLine(
							positions[i],
							positions[(i + 1) % length],
							minDistance,
					...
					tempPositions =
						PolygonGeometryLibrary.PolygonGeometryLibrary.subdivideRhumbLine(
							ellipsoid,
							positions[i],
							positions[(i + 1) % length],
							minDistance,
	...
	PolygonOutlineGeometry.createGeometry = function (polygonGeometry) {
		...
		var polygons =
			PolygonGeometryLibrary.PolygonGeometryLibrary.polygonOutlinesFromHierarchy(
				polygonHierarchy,
				!perPositionHeight,
				ellipsoid,
			)
