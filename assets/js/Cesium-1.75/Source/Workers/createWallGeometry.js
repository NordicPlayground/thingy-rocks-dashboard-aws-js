...
define([
	...
	function WallGeometry(options) {
		...
		var numComponents =
			1 + wallPositions.length * Cartesian2.Cartesian3.packedLength + 2
	...
	WallGeometry.createGeometry = function (wallGeometry) {
		...
		for (i = 0; i < numVertices - 2; i += 2) {
			...
			if (
				Cartesian2.Cartesian3.equalsEpsilon(pl, pr, _Math.CesiumMath.EPSILON10)
			) {
				continue
			}
			var UL = i + 1
	...
	function createWallGeometry(wallGeometry, offset) {
		...
		wallGeometry._ellipsoid = Cartesian2.Ellipsoid.clone(
			wallGeometry._ellipsoid,
		)
