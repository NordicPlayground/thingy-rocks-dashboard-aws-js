...
define([
	...
	PrimitivePipeline.combineGeometry = function (parameters) {
		...
		for (var i = 0; i < length; ++i) {
			...
			if (
				when.defined(eastHemisphereGeometry) &&
				when.defined(westHemisphereGeometry)
			) {
				if (
	...
	PrimitivePipeline.packCreateGeometryResults = function (
		...
		for (var i = 0; i < length; i++) {
			...
			var validBoundingSphere = when.defined(geometry.boundingSphere)
				? 1.0
				: 0.0
			...
			var validBoundingSphereCV = when.defined(geometry.boundingSphereCV)
				? 1.0
				: 0.0
			...
			var indicesLength = when.defined(geometry.indices)
	...
	function packInstancesForCombine(instances, transferableObjects) {
		...
		for (var i = 0; i < length; i++) {
			...
			if (
				when.defined(instance.attributes) &&
				when.defined(instance.attributes.offset)
			) {
