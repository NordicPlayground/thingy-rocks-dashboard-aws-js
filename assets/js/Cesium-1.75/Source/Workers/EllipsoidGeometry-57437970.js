...
define([
	...
	function EllipsoidGeometry(options) {
		...
		var maximumClock = when.defaultValue(
			options.maximumClock,
			_Math.CesiumMath.TWO_PI,
		)
		var minimumCone = when.defaultValue(options.minimumCone, 0.0)
		var maximumCone = when.defaultValue(
			options.maximumCone,
			_Math.CesiumMath.PI,
		)
		var stackPartitions = Math.round(
			when.defaultValue(options.stackPartitions, 64),
		)
		var slicePartitions = Math.round(
			when.defaultValue(options.slicePartitions, 64),
		)
	...
	EllipsoidGeometry.unpack = function (array, startingIndex, result) {
		...
		var innerRadii = Cartesian2.Cartesian3.unpack(
			array,
			startingIndex,
			scratchInnerRadii,
		)
		...
		result._innerRadii = Cartesian2.Cartesian3.clone(
			innerRadii,
			result._innerRadii,
		)
	...
	EllipsoidGeometry.createGeometry = function (ellipsoidGeometry) {
		...
		var isInner = GeometryOffsetAttribute.arrayFill(
			new Array(vertexCount),
			false,
		)
		var negateNormal = GeometryOffsetAttribute.arrayFill(
			new Array(vertexCount),
			false,
		)
			...
			for (i = 0; i < vertexCount; i++) {
				...
				if (vertexFormat.tangent || vertexFormat.bitangent) {
					...
					if (vertexFormat.bitangent) {
						var bitangent = Cartesian2.Cartesian3.cross(
							normal,
							tangent,
							scratchBitangent,
						)
