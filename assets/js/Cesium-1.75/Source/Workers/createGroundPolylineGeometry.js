...
define([
	...
	'./ComponentDatatype-cb08e294',
	'./GeometryAttribute-7fce7c2c',
	'./EncodedCartesian3-46a75b7e',
	'./IntersectionTests-9b2b2e4d',
	'./Plane-f21775c7',
	'./WebMercatorProjection-ee5bc563',
	'./arrayRemoveDuplicates-b37f47ad',
	'./ArcType-13a53523',
	'./EllipsoidRhumbLine-6133712f',
	'./EllipsoidGeodesic-5479ce7e',
], function (
	when,
	Check,
	_Math,
	...
	function GeographicTilingScheme(options) {
		...
		this._rectangle = when.defaultValue(
			options.rectangle,
			Cartesian2.Rectangle.MAX_VALUE,
		)
	 ...
	 */
	ApproximateTerrainHeights.getBoundingSphere = function (
		rectangle,
		ellipsoid,
	) {
		//>>includeStart('debug', pragmas.debug);
		...
		var result = Transforms.BoundingSphere.fromRectangle3D(
			rectangle,
			ellipsoid,
			0.0,
		)
		...
		return Transforms.BoundingSphere.union(
			result,
			scratchBoundingSphere,
			result,
		)
	...
	var PROJECTIONS = [
		Transforms.GeographicProjection,
	...
	function interpolateSegment(
		...
		if (arcType === ArcType.ArcType.GEODESIC) {
			ellipsoidLine = new EllipsoidGeodesic.EllipsoidGeodesic(
				start,
				end,
				ellipsoid,
			)
		} else if (arcType === ArcType.ArcType.RHUMB) {
			ellipsoidLine = new EllipsoidRhumbLine.EllipsoidRhumbLine(
				start,
				end,
				ellipsoid,
			)
	...
	GroundPolylineGeometry.createGeometry = function (groundPolylineGeometry) {
		...
		var c0
		var c1
		var rhumbLine = new EllipsoidRhumbLine.EllipsoidRhumbLine(
			undefined,
			undefined,
			ellipsoid,
		)
		...
		for (i = 0; i < cartographicsLength; i++) {
			var cartographic = Cartesian2.Cartographic.fromCartesian(
				splitPositions[i],
				ellipsoid,
			)
	...
	function projectNormal(
		...
		var normalEndpoint = Cartesian2.Cartesian3.add(
			position,
			normal,
	...
	var scratchBoundingSpheres = [
		new Transforms.BoundingSphere(),
		new Transforms.BoundingSphere(),
	]
	...
	function generateGeometryAttributes(
		...
		for (i = 1; i < positionsLength; i++) {
			...
			length3D += Cartesian2.Cartesian3.distance(
				segmentStartCartesian,
				segmentEndCartesian,
			)
			index += 3
		...
		var endTop = Cartesian2.Cartesian3.unpack(
			topPositionsArray,
			0,
		...
		for (i = 0; i < segmentCount; i++) {
			var startBottom = Cartesian2.Cartesian3.clone(
				endBottom,
				segmentStartBottomScratch,
			)
			...
			endTop = Cartesian2.Cartesian3.unpack(
				topPositionsArray,
				index,
				segmentEndTopScratch,
			)
			...
			if (compute2dAttributes) {
				...
				if (
					nudgeResult === 0 ||
					Cartesian2.Cartesian3.dot(direction2D, Cartesian2.Cartesian3.UNIT_Y) >
						MITER_BREAK_SMALL
				) {
			...
			var startUp = Cartesian2.Cartesian3.subtract(
				startTop,
				startBottom,
				startUpScratch,
			)
			startUp = Cartesian2.Cartesian3.normalize(startUp, startUp)
			var rightNormal = Cartesian2.Cartesian3.cross(
				forward,
				startUp,
				rightScratch,
			)
			...
			startPlaneNormal = Cartesian2.Cartesian3.normalize(
				startPlaneNormal,
				startPlaneNormal,
			)

			var endUp = Cartesian2.Cartesian3.subtract(
				endTop,
				endBottom,
				endUpScratch,
			)
			...
			endPlaneNormal = Cartesian2.Cartesian3.normalize(
				endPlaneNormal,
				endPlaneNormal,
			)
			...
			if (compute2dAttributes) {
				...
				right2D = Cartesian2.Cartesian3.normalize(
					forwardOffset2D,
					right2DScratch,
				)
				var swap = right2D.x
		...
		var boundingSphere =
			Transforms.BoundingSphere.fromBoundingSpheres(boundingSpheres)
		...
		if (compute2dAttributes) {
			...
			attributes.texcoordNormalization2D =
