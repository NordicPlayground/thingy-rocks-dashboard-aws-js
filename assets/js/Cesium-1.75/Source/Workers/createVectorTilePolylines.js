...
define([
	...
	function decodePositions(
		...
		for (var i = 0; i < positionsLength; ++i) {
			...
			var lon = _Math.CesiumMath.lerp(
				rectangle.west,
				rectangle.east,
				u / maxShort,
			)
			var lat = _Math.CesiumMath.lerp(
				rectangle.south,
				rectangle.north,
				v / maxShort,
			)
			var alt = _Math.CesiumMath.lerp(
				minimumHeight,
				maximumHeight,
				h / maxShort,
			)
	...
	function createVectorTilePolylines(parameters, transferableObjects) {
		...
		for (i = 0; i < length; ++i) {
			...
			for (var j = 0; j < count; ++j) {
				var previous
				if (j === 0) {
					var p0 = Cartesian2.Cartesian3.unpack(
						positions,
						offset * 3,
						scratchP0,
					)
					var p1 = Cartesian2.Cartesian3.unpack(
						positions,
						(offset + 1) * 3,
						scratchP1,
					)
				...
				var current = Cartesian2.Cartesian3.unpack(
					positions,
					(offset + j) * 3,
					scratchCur,
				)
				...
				if (j === count - 1) {
					...
					next = Cartesian2.Cartesian3.unpack(
						positions,
						(offset + j + 1) * 3,
						scratchNext,
					)
