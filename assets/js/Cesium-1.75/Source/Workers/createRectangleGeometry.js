...
define([
	...
	'./GeometryPipeline-8b1bcc9c',
	'./EncodedCartesian3-46a75b7e',
	'./IndexDatatype-7948b6db',
	'./IntersectionTests-9b2b2e4d',
	'./Plane-f21775c7',
	'./GeometryOffsetAttribute-9c46b133',
	'./VertexFormat-2df57ea4',
	'./GeometryInstance-d4a7f55f',
	'./EllipsoidRhumbLine-6133712f',
	'./PolygonPipeline-2140d99c',
	'./RectangleGeometryLibrary-d24aaefb',
], function (
	when,
	Check,
	_Math,
	...
	function calculateAttributesWall(positions, vertexFormat, ellipsoid) {
		...
		if (vertexFormat.normal || vertexFormat.tangent || vertexFormat.bitangent) {
			for (var i = 0; i < length; i += 6) {
				...
				if (
					Cartesian2.Cartesian3.equalsEpsilon(p1, p, _Math.CesiumMath.EPSILON10)
				) {
	...
	function constructExtrudedRectangle(rectangleGeometry, computedOptions) {
		...
		if (shadowVolume) {
			var newVertexFormat = VertexFormat.VertexFormat.clone(
				vertexFormat,
				scratchVertexFormat,
			)
		...
		if (hasOffsets && !computeTopOffsets) {
			offsetValue =
				...
				GeometryOffsetAttribute.GeometryOffsetAttribute.ALL
					? 1
					: 0
			wallOffsetAttribute = GeometryOffsetAttribute.arrayFill(
				wallOffsetAttribute,
				offsetValue,
			)
		}
		...
		for (i = 0; i < length - 1; i += 2) {
			...
			if (
				Cartesian2.Cartesian3.equalsEpsilon(p1, p2, _Math.CesiumMath.EPSILON10)
			) {
				continue
			}
	...
	RectangleGeometry.unpack = function (array, startingIndex, result) {
		...
		var rectangle = Cartesian2.Rectangle.unpack(
			array,
			startingIndex,
			scratchRectangle,
		)
	...
	RectangleGeometry.createGeometry = function (rectangleGeometry) {
		...
		if (stRotation !== 0 || rotation !== 0) {
			...
			Transforms.Matrix3.fromQuaternion(
				quaternionScratch,
				tangentRotationMatrix,
			)
		} else {
			Transforms.Matrix3.clone(
	...
	function textureCoordinateRotationPoints(rectangleGeometry) {
		...
		for (var i = 0; i < 3; ++i) {
			...
			GeometryAttribute.Matrix2.multiplyByVector(
				toDesiredInComputed,
				point2D,
				point2D,
			)
	...
	function createRectangleGeometry(rectangleGeometry, offset) {
		...
		rectangleGeometry._ellipsoid = Cartesian2.Ellipsoid.clone(
			rectangleGeometry._ellipsoid,
		)
		rectangleGeometry._rectangle = Cartesian2.Rectangle.clone(
			rectangleGeometry._rectangle,
		)
