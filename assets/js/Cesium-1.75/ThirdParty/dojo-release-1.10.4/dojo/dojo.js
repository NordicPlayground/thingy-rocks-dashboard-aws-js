;(function (userConfig, defaultConfig) {
	...
	var injectDependencies = function (module) {
		...
		getModuleInfo = function (mid, referenceModule, fromPendingCache) {
			return getModuleInfo_(
				...
				modules,
				req.baseUrl,
				fromPendingCache ? [] : mapProgs,
				fromPendingCache ? [] : pathsMapProg,
				fromPendingCache ? [] : aliases,
			)
		},
		...
		getModule = function (mid, referenceModule, immediate) {
			...
			if (match) {
				...
				if (plugin.load) {
					...
					prid = match[2]
					mid =
						plugin.mid + '!' + ++dynamicPluginUidGenerator + '!waitingForPlugin'
				}
				result = {
					plugin: plugin,
					mid: mid,
			...
			return (
				modules[result.mid] || (!immediate && (modules[result.mid] = result))
			)
		},
				...
				if (has('config-dojo-loader-catches')) {
					try {
						if (text === cached) {
							...
							req.eval(
								text,
								has('dojo-loader-eval-hint-url') ? module.url : module.mid,
							)
						}
					} catch (e) {
							...
							if (legacyMode == sync) {
								...
								if (module.finish) {
									...
									def(
										...
										[
											'dojo',
											('dojo/require!' + finish.join(',')).replace(/\./g, '/'),
										],
										function (dojo) {
	...
	var def = function (
		...
		if (targetModule && !waiting[targetModule.mid]) {
			...
			!has('ie-event-behavior') ||
			!has('host-browser') ||
			injectingCachedModule
		) {
