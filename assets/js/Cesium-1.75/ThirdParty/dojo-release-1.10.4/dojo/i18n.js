define([
	...
	var thisModule = (dojo.i18n = {
		...
		getAvailableLocales = function (root, locale, bundlePath, bundleName) {
			...
			for (
				var result = [bundlePath + bundleName],
		...
		getL10nName = (dojo.getL10nName = function (
			moduleName,
			bundleName,
			locale,
		) {
			return (moduleName =
				module.id + '!' + getBundleName(moduleName, bundleName, locale))
		}),
		doLoad = function (
			require,
			bundlePathAndName,
							...
							doRequire(mid, function (rollup) {
								for (var p in rollup) {
									...
									if (loc === 'ROOT') {
										...
										cache[cacheId(bundlePath, bundleName, loc, require)] =
											bundle
									}

									if (loc !== locale) {
										// capture some locale variables
										function improveBundle(
											bundlePath,
											bundleName,
											bundle,
											localized,
											...
											forEachLocale(locale, function (loc) {
												if (localized[loc]) {
													requiredBundles.push(
														require.toAbsMid(
															bundlePath + loc + '/' + bundleName,
														),
													)
													cacheIds.push(
														cacheId(bundlePath, bundleName, loc, require),
													)
												}
											})
											...
											if (requiredBundles.length) {
												...
												contextRequire(requiredBundles, function () {
													...
													cache[
														cacheId(bundlePath, bundleName, locale, require)
													] = lang.clone(bundle)
													preloadingRelLock()
												})
											} else {
					...
					function load(text) {
						...
						if (result === amdValue) {
							...
							if (result instanceof Error) {
								console.error(
									'failed to evaluate i18n bundle; url=' + url,
									result,
								)
								result = {}
							}
