define([
	...
	return declare('dojo.store.DataStore', base, {
		...
		put: function (object, options) {
			...
			options = options || {}
			var id =
				typeof options.id != 'undefined' ? options.id : this.getIdentity(object)
			var store = this.store
			var idProperty = this.idProperty
			var deferred = new Deferred()
						...
						if (item) {
							if (options.overwrite === false) {
								return deferred.reject(
									new Error('Overwriting existing object not allowed'),
								)
							}
							for (var i in object) {
								if (
									i != idProperty && // don't copy id properties since they are immutable and should be omitted for implicit ids
							...
							if (options.overwrite === true) {
								return deferred.reject(
									new Error('Creating new object not allowed'),
								)
							}
							store.newItem(object)
						}
