define(['./sniff', './_base/window', './dom', './dom-style'], function (
	...
	if (has('ie') /*|| has("opera")*/) {
		...
		geom.boxModel =
			document.compatMode == 'BackCompat' ? 'border-box' : 'content-box'
	}
	...
	function isButtonTag(/*DomNode*/ node) {
		...
		return (
			...
			(node.tagName.toLowerCase() == 'input' &&
				(node.getAttribute('type') || '').toLowerCase() == 'button')
		) // boolean
	}
	...
	geom.isBodyLtr = function isBodyLtr(/*Document?*/ doc) {
		...
		doc = doc || win.doc
		return (
			(win.body(doc).dir || doc.documentElement.dir || 'ltr').toLowerCase() ==
			'ltr'
		) // Boolean
	}
	...
	geom.fixIeBiDiScrollLeft = function fixIeBiDiScrollLeft(
		...
		if (ie && !geom.isBodyLtr(doc)) {
			...
			return ie < 8 || qk
				? scrollLeft + de.clientWidth - de.scrollWidth
				: -scrollLeft // Integer
		}
	...
	geom.normalizeEvent = function (event) {
		...
		if (!has('dom-addeventlistener')) {
			...
			event.pageX =
				event.clientX +
				geom.fixIeBiDiScrollLeft(docBody.scrollLeft || 0, doc) -
				offset.x
