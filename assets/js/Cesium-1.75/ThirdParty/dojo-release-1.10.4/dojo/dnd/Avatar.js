define([
	...
	return declare('dojo.dnd.Avatar', null, {
		...
		construct: function () {
			...
			for (; i < k; ++i) {
				if (source.creator) {
					...
					node = source._normalizedCreator(
						source.getItem(this.manager.nodes[i].id).data,
						'avatar',
					).node
				} else {
		...
		update: function () {
			...
			// replace text
			query(
				'tr.dojoDndAvatarHeader td span' + (has('highcontrast') ? ' span' : ''),
				this.node,
