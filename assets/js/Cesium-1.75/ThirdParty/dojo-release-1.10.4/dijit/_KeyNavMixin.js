	...
	return declare('dijit._KeyNavMixin', _FocusMixin, {
		...
		postCreate: function () {
			...
			if (!this._keyNavCodes) {
				...
				keyCodes[this.isLeftToRight() ? keys.LEFT_ARROW : keys.RIGHT_ARROW] =
					lang.hitch(this, '_onLeftArrow')
				keyCodes[this.isLeftToRight() ? keys.RIGHT_ARROW : keys.LEFT_ARROW] =
					lang.hitch(this, '_onRightArrow')
			...
			var self = this,
				childSelector =
					typeof this.childSelector == 'string'
						? this.childSelector
			...
			this.own(
				...
				on(
					this.containerNode,
					on.selector(childSelector, 'focusin'),
					function (evt) {
		...
		_keyboardSearchCompare: function (
			...
			var element = item.domNode,
				text =
					item.label ||
					(element.focusNode ? element.focusNode.label : '') ||
					element.innerText ||
					element.textContent ||
					'',
				currentString = text
		...
		_onChildBlur: function () /*dijit/_WidgetBase*/
		/*===== widget =====*/ {
			// summary:
