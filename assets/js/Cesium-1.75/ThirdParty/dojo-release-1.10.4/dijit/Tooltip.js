			...
			show: function (
				...
				if (pos.corner.charAt(0) == 'M' && pos.aroundCorner.charAt(0) == 'M') {
					this.connectorNode.style.top =
						aroundNodeCoords.y +
						((aroundNodeCoords.h - this.connectorNode.offsetHeight) >> 1) -
						pos.y +
						'px'
					this.connectorNode.style.left = ''
				} else if (
					pos.corner.charAt(1) == 'M' &&
					pos.aroundCorner.charAt(1) == 'M'
				) {
					this.connectorNode.style.left =
						aroundNodeCoords.x +
						((aroundNodeCoords.w - this.connectorNode.offsetWidth) >> 1) -
						pos.x +
						'px'
				} else {
			...
			orient: function (
				...
				if (tooltipCorner.charAt(0) == 'B' && aroundCorner.charAt(0) == 'B') {
					...
					if (bb.h > heightAvailable) {
						...
						var aroundNodePlacement =
							heightAvailable -
							((aroundNodeCoords.h + tooltipConnectorHeight) >> 1)
						...
						this.connectorNode.style.bottom =
							Math.min(
								Math.max(
									aroundNodeCoords.h / 2 - tooltipConnectorHeight / 2,
									0,
								),
								bb.h - tooltipConnectorHeight,
							) + 'px'
						this.connectorNode.style.top = ''
					}
				} else {
	...
	dijit.showTooltip = function (
		...
		if (position) {
			position = array.map(position, function (val) {
				return (
					{ after: 'after-centered', before: 'before-centered' }[val] || val
				)
			})
		}
			...
			this._connections = array.map(
				...
				function (id) {
					...
					return [
						...
						on(
							node,
							delegatedEvent(mouse.leave),
							lang.hitch(self, '_onUnHover'),
						),
						on(
							node,
							delegatedEvent('focusout'),
							lang.hitch(self, 'set', 'state', DORMANT),
						),
					]
				},
				this,
			...
			switch (val) {
				...
				case SHOWING: // show tooltip and clear timers
					...
					Tooltip.show(
						...
						lang.hitch(this, 'set', 'state', HIDE_TIMER),
					)
