define([
	...
	var SelectionManager = function (win) {
		...
		this.getType = function () {
			...
			if (doc.getSelection) {
				...
				if (oSel && oSel.rangeCount == 1) {
					...
					if (
						oRange.startContainer == oRange.endContainer &&
		...
		this.selectElementChildren = function (
			/*DomNode*/ element,
			...
			if (doc.getSelection) {
				...
				if (has('opera')) {
					...
					range.setEnd(
						element,
						element.nodeType == 3 ? element.length : element.childNodes.length,
					)
