define([
	...
	var InlineEditor = declare(
		...
		[_Widget, _TemplatedMixin, _WidgetsInTemplateMixin],
		{
			...
			buildRendering: function () {
				...
				var Cls =
					typeof this.editor == 'string'
						? lang.getObject(this.editor) || require(this.editor)
						: this.editor
				...
				if (width == '100%') {
					...
					// inline-block mode
					editStyle +=
						'width:' + (width + (Number(width) == width ? 'px' : '')) + ';'
				}
					...
					if ('intermediateChanges' in ew) {
						...
						this.own(
							aspect.after(
								ew,
								'onChange',
								lang.hitch(this, '_onIntermediateChange'),
								true,
							),
						)
			...
			getValue: function () {
				...
				return String(
					ew.get(
						'displayedValue' in ew || '_getDisplayedValueAttr' in ew
							? 'displayedValue'
							: 'value',
					),
				)
	...
	var InlineEditBox = declare(
		'dijit.InlineEditBox' + (has('dojo-bidi') ? '_NoBidi' : ''),
		_Widget,
		{
			// summary:
			...
			postMixInProperties: function () {
				...
				this.own(
					...
					on(
						this.displayNode,
						'mouseover, focus',
						lang.hitch(this, '_onMouseOver'),
					),
					on(
						this.displayNode,
						'mouseout, blur',
						lang.hitch(this, '_onMouseOut'),
					),
				)
			...
			edit: function () {
				...
				if (!this.wrapperWidget) {
					...
					var placeholder = domConstruct.create(
						'span',
						null,
						this.domNode,
						'before',
					...
					var Ewc =
						typeof this.editorWrapper == 'string'
							? lang.getObject(this.editorWrapper)
							: this.editorWrapper
				...
				when(
					ew.onLoadDeferred,
					lang.hitch(ww, function () {
						// set value again in case the edit widget's value is just now valid
						ew.set(
							'displayedValue' in ew || '_setDisplayedValueAttr' in ew
								? 'displayedValue'
								: 'value',
							self.value,
						)
			...
			_setValueAttr: function (/*String*/ val) {
				...
				val = lang.trim(val)
				var renderVal = this.renderAsHtml
					? val
					: val
							.replace(/&/gm, '&amp;')
							.replace(/</gm, '&lt;')
