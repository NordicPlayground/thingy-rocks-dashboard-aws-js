define([
	...
	var _SelectMenu = declare('dijit.form._SelectMenu', DropDownMenu, {
		...
		postCreate: function () {
			...
			this.own(
				on(this.domNode, 'selectstart', function (evt) {
					evt.preventDefault()
					evt.stopPropagation()
				}),
			)
	...
	var Select = declare(
		'dijit.form.Select' + (has('dojo-bidi') ? '_NoBidi' : ''),
		[_FormSelectWidget, _HasDropDown, _KeyNavMixin],
		{
			// summary:
			...
			_getMenuItemForOption: function (
				/*_FormSelectWidget.__SelectOption*/ option,
			) {
				// summary:
			...
			_setDisplay: function (/*String*/ newDisplay) {
				...
				var lbl =
					(this.labelType === 'text'
						? (newDisplay || '').replace(/&/g, '&amp;').replace(/</g, '&lt;')
			...
			validate: function (/*Boolean*/ isFocused) {
				...
				this._set(
					'state',
					isValid ? '' : this._hasBeenBlurred ? 'Error' : 'Incomplete',
				)
			...
			postMixInProperties: function () {
				...
				this.inherited(arguments)
				this._missingMsg = i18n.getLocalization(
					'dijit.form',
					'validate',
					this.lang,
				).missingMessage
			...
			_setStyleAttr: function (/*String||Object*/ value) {
				...
				domClass.toggle(
					this.domNode,
					this.baseClass.replace(/\s+|$/g, 'FixedWidth '),
	...
	Select.prototype._onContainerKeydown = _onEventAfterLoad(
		'_onContainerKeydown',
	)
	Select.prototype._onContainerKeypress = _onEventAfterLoad(
