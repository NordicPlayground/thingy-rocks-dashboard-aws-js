define([
	...
	var MultiSelect = declare(
		'dijit.form.MultiSelect' + (has('dojo-bidi') ? '_NoBidi' : ''),
		_FormValueWidget,
		{
			// summary:
			...
			templateString:
				"<select multiple='multiple' ${!nameAttrSetting} data-dojo-attach-point='containerNode,focusNode' data-dojo-attach-event='onchange: _onChange'></select>",
			...
			_setValueAttr: function (/*String[]*/ values) {
				...
				if (has('android')) {
					...
					query('option', this.containerNode)
						.orphan()
						.forEach(function (n) {
	...
	if (has('dojo-bidi')) {
		MultiSelect = declare('dijit.form.MultiSelect', MultiSelect, {
			addSelected: function (/*dijit/form/MultiSelect*/ select) {
				select.getSelected().forEach(function (n) {
					n.text = this.enforceTextDirWithUcc(
						this.restoreOriginalText(n),
						n.text,
					)
				}, this)
				this.inherited(arguments)
			...
			_setTextDirAttr: function (textDir) {
				...
				if (
					(this.textDir != textDir || !this._created) &&
					...
					query('option', this.containerNode).forEach(function (option) {
						...
						option.text = this.enforceTextDirWithUcc(
							option,
