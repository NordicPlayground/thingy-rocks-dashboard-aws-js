	...
	var RangeBoundTextBox = declare(
		...
		======*/
			...
			rangeCheck: function (
				...
				return (
					('min' in constraints
						? this.compare(primitive, constraints.min) >= 0
						: true) &&
					('max' in constraints
						? this.compare(primitive, constraints.max) <= 0
						: true)
				) // Boolean
			},

			isInRange: function () /*Boolean*/
			/*===== isFocused =====*/ {
				// summary:
			...
			_isDefinitelyOutOfRange: function () {
				...
				if (!outOfRange && 'max' in this.constraints) {
					...
					outOfRange =
						this.compare(val, typeof max != 'number' || max > 0 ? max : 0) > 0
				}
				return outOfRange
			},
