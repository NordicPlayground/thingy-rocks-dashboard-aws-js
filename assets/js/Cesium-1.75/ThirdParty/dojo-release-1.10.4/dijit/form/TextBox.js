define([
	...
	var TextBox = declare(
		'dijit.form.TextBox' + (has('dojo-bidi') ? '_NoBidi' : ''),
		[_FormValueWidget, _TextBoxMixin],
		{
			// summary:
			...
			postMixInProperties: function () {
				...
				if (
					...
					((type == 'hidden' || type == 'file') &&
						this.templateString == this.constructor.prototype.templateString)
				) {
			...
			_setPlaceHolderAttr: function (v) {
				...
				if (!this._phspan) {
					...
					this._phspan = domConstruct.create(
						'span',
						{
					...
					this.own(
						on(this._phspan, 'mousedown', function (evt) {
							evt.preventDefault()
						}),
						on(
							this._phspan,
			...
			_updatePlaceHolder: function () {
				if (this._phspan) {
					this._phspan.style.display =
						this.placeHolder && !this.textbox.value ? '' : 'none'
				}
			},
