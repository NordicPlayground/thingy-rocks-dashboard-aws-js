define([
	...
	var AutoCompleterMixin = declare(
		'dijit.form._AutoCompleterMixin',
		SearchMixin,
		{
			// summary:
			...
			_autoCompleteText: function (/*String*/ text) {
				...
				if (
					text[caseFilter](0).indexOf(this.focusNode.value[caseFilter](0)) == 0
				) {
			...
			_openResultList: function (
				...
				this._nextSearch = this.dropDown.onPage = lang.hitch(
					this,
					function (direction) {
				...
				if ('direction' in options) {
					...
					this._announceOption(
						this.dropDown.containerNode.firstChild.nextSibling,
					) // 1st real item
				}
			},
			...
			loadDropDown: function () /*Function*/
			/*===== callback =====*/ {
				...
				if (item) {
					if (!displayedValue) {
						displayedValue = this.store._oldAPI // remove getValue() for 2.0 (old dojo.data API)
							? this.store.getValue(item, this.searchAttr)
							: item[this.searchAttr]
					}
					value =
			...
			_announceOption: function (/*Node*/ node) {
				...
				if (
					...
					newValue = (
						this.store._oldAPI // remove getValue() for 2.0 (old dojo.data API)
							? this.store.getValue(item, this.searchAttr)
							: item[this.searchAttr]
					).toString()
				...
				if (!this.dropDown) {
					var popupId = this.id + '_popup',
						dropDownConstructor = lang.isString(this.dropDownClass)
							? lang.getObject(this.dropDownClass, false)
							: this.dropDownClass
					...
					if (!('value' in this.params)) {
						...
						if (item) {
							...
							this.value = this.store._oldAPI
								? this.store.getValue(item, valueField)
								: item[valueField]
						}
					}
				}
			...
			postCreate: function () {
				...
				aspect.after(
					this,
					'onSearch',
					lang.hitch(this, '_openResultList'),
					true,
				)
			},
			...
			labelFunc: function (item, store) {
				...
				return (
					store._oldAPI
						? store.getValue(item, this.labelAttr || this.searchAttr)
						: item[this.labelAttr || this.searchAttr]
				).toString() // String
			},
