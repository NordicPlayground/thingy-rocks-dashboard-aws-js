define([
	...
	var HorizontalRuleLabels = declare(
		'dijit.form.HorizontalRuleLabels',
		HorizontalRule,
		{
			// summary:
					...
					for (var i = 0; i < this.count; i++) {
						labels.push(
							i < this.numericMargin || i >= this.count - this.numericMargin
								? ''
								: number.format(start, this.constraints),
						)
						start += inc
					}
				}
				...
				if (this.textDir != textDir) {
					...
					query('.dijitRuleLabelContainer', this.domNode).forEach(
						lang.hitch(this, function (labelNode) {
							labelNode.style.direction = this.getTextDir(
