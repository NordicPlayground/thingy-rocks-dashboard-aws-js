define([
	...
	var _TextBoxMixin = declare(
		'dijit.form._TextBoxMixin' + (has('dojo-bidi') ? '_NoBidi' : ''),
		null,
		{
			// summary:
			...
			_setValueAttr: function (
				...
				if (value !== undefined) {
					...
					if (typeof formattedValue != 'string') {
						if (
							...
							formattedValue = this.filter(
								this.format(filteredValue, this.constraints),
							)
						} else {
							formattedValue = ''
						}
						...
						if (
							this.compare(
								filteredValue,
								this.filter(this.parse(formattedValue, this.constraints)),
							) != 0
						) {
							formattedValue = null
						}
				...
				if (
					formattedValue != null /* and !undefined */ &&
				...
				function handleEvent(e) {
					...
					if (e.type == 'keydown') {
						...
						if (!e.ctrlKey && !e.metaKey && !e.altKey) {
							// no modifiers
							switch (
								charOrCode // ignore location keys
							) {
								case keys.NUMPAD_0:
								case keys.NUMPAD_1:
								case keys.NUMPAD_2:
							...
							if (
								(charOrCode >= 65 && charOrCode <= 90) ||
								(charOrCode >= 48 && charOrCode <= 57) ||
								charOrCode == keys.SPACE
							) {
					...
					charOrCode =
						e.charCode >= 32 ? String.fromCharCode(e.charCode) : e.charCode
					if (!charOrCode) {
						charOrCode =
					...
					if (e.type == 'keypress') {
						...
						if (
							(charOrCode >= 'a' && charOrCode <= 'z') ||
							(charOrCode >= 'A' && charOrCode <= 'Z') ||
							(charOrCode >= '0' && charOrCode <= '9') ||
							charOrCode === ' '
						) {
							if (e.ctrlKey || e.metaKey || e.altKey) {
								return
			...
			_onFocus: function (/*String*/ by) {
				...
				if (this.selectOnClick && by == 'mouse') {
					...
					// the selection.
					this._selectOnClickHandle = on.once(
						this.domNode,
						'mouseup, touchend',
