	...
	return declare('dijit.form._FormWidgetMixin', null, {
		...
		_onFocus: function (/*String*/ by) {
			...
			if (by == 'mouse' && this.isFocusable()) {
				...
				var focusHandle = this.own(
					on(this.focusNode, 'focus', function () {
				...
				var event = has('pointer-events')
					? 'pointerup'
					: has('MSPointer')
					? 'MSPointerUp'
					: has('touch-events')
					? 'touchend, mouseup' // seems like overkill but see #16622, #16725
					: 'mouseup'
				var mouseUpHandle = this.own(
					on(
		...
		isFocusable: function () {
			...
			return (
				!this.disabled &&
				this.focusNode &&
				domStyle.get(this.domNode, 'display') != 'none'
			)
		},
		...
		_handleOnChange: function (
			...
			if (
				...
				(priorityChange === null || !this._onChangeActive)
			) {
