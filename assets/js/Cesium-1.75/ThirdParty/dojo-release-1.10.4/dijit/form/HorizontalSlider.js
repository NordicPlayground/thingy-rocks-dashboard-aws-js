define([
	...
	var _SliderMover = declare('dijit.form._SliderMover', Mover, {
		onMouseMove: function (e) {
			...
			if (!abspos) {
				abspos = widget._abspos = domGeometry.position(
					widget.sliderBarContainer,
					true,
				)
			...
			var pixelValue =
				e[widget._mousePixelCoord] - abspos[widget._startingPixelCoord]
			widget._setPixelValue_(
				widget._isReversed_
					? abspos[widget._pixelCount] - pixelValue
					: pixelValue,
				abspos[widget._pixelCount],
				false,
			)
		},
	...
	var HorizontalSlider = declare(
		'dijit.form.HorizontalSlider',
		[_FormValueWidget, _Container],
		{
			// summary:
			...
			_onBarClick: function (e) {
				...
				var pixelValue =
					e[this._mousePixelCoord] - abspos[this._startingPixelCoord]
				this._setPixelValue(
					this._isReversed()
						? abspos[this._pixelCount] - pixelValue
						: pixelValue,
					abspos[this._pixelCount],
					true,
				)
				this._movable.onMouseDown(e)
			},

			_setPixelValue: function (
				/*Number*/ pixelValue,
				/*Number*/ maxPixels,
				...
				this._setValueAttr(
					Math.max(
						Math.min(
							((this.maximum - this.minimum) * wholeIncrements) / count +
								this.minimum,
							this.maximum,
						),
						this.minimum,
			...
			_setValueAttr: function (/*Number*/ value, /*Boolean?*/ priorityChange) {
				...
				if (
					...
					progressBar.style[this._progressPixelSize]
				) {
					// animate the slider
					var _this = this
					var props = {}
					...
					this._inProgressAnim = fx.animateProperty({
						...
						onAnimate: function (v) {
							remainingBar.style[_this._progressPixelSize] =
								100 - parseFloat(v[_this._progressPixelSize]) + '%'
						},
						onEnd: function () {
					...
					remainingBar.style[this._progressPixelSize] =
						(1 - percent) * 100 + '%'
				}
			},
