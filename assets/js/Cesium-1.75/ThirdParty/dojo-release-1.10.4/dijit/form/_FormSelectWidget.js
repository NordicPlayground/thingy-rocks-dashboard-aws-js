	...
	var _FormSelectWidget = declare(
		...
		_FormValueWidget,
		{
			...
			_setStoreAttr: function (val) {
				if (this._created) {
					// don't repeat work that will happen in postCreate()
			...
			getOptions: function (/*anything*/ valueOrIdx) {
				...
				if (lang.isObject(valueOrIdx)) {
					...
					if (
						!array.some(opts, function (option, idx) {
			...
			addOption: function (/*__SelectOption|__SelectOption[]*/ option) {
				...
				array.forEach(
					lang.isArray(option) ? option : [option],
					function (i) {
			...
			removeOption: function (
				/*String|__SelectOption|Number|Array*/ valueOrIdx,
			) {
				// summary:
				...
				var oldOpts = this.getOptions(
					lang.isArray(valueOrIdx) ? valueOrIdx : [valueOrIdx],
				)
				array.forEach(
					oldOpts,
					function (option) {
						...
						if (option) {
							this.options = array.filter(this.options, function (node) {
								return (
									node.value !== option.value || node.label !== option.label
								)
							})
			...
			updateOption: function (/*__SelectOption|__SelectOption[]*/ newOption) {
				...
				array.forEach(
					lang.isArray(newOption) ? newOption : [newOption],
					function (i) {
						var oldOpt = this.getOptions({ value: i.value }),
							k
						if (oldOpt) {
							for (k in i) {
								oldOpt[k] = i[k]
						...
						if (store.getFeatures()['dojo.data.api.Notification']) {
							this._notifyConnections = [
								aspect.after(
									store,
									'onNew',
									lang.hitch(this, '_onNewItem'),
									true,
								),
								aspect.after(
									store,
									'onDelete',
									lang.hitch(this, '_onDeleteItem'),
									true,
								),
								aspect.after(
									store,
									'onSet',
									lang.hitch(this, '_onSetItem'),
									true,
								),
							]
						}
					...
					when(
						...
						lang.hitch(this, function (items) {
							if (this.sortByLabel && !fetchArgs.sort && items.length) {
								if (store.getValue) {
									...
									items.sort(function (a, b) {
										return a[labelAttr] > b[labelAttr]
											? 1
											: b[labelAttr] > a[labelAttr]
											? -1
											: 0
									})
								}
							...
							if (this._queryRes.observe) {
								...
								this._observeHandle = this._queryRes.observe(
									lang.hitch(
										this,
										function (object, deletedFrom, insertedInto) {
							...
							this.set(
								'value',
								'_pendingValue' in this ? this._pendingValue : selectedValue,
							)
			...
			_setDisplayedValueAttr: function (label) {
				...
				this.set(
					'value',
					this.getOptions(typeof label == 'string' ? { label: label } : label),
				)
			},
				...
				if (val && val[0]) {
					...
					array.forEach(
						...
						function (child) {
							...
							domClass.toggle(
								child.domNode,
								this.baseClass.replace(/\s+|$/g, 'SelectedOption '),
								isSelected,
							)
							child.domNode.setAttribute(
								'aria-selected',
			...
			_getOptionObjForItem: function (item) {
				...
				var store = this.store,
					label =
						this.labelAttr && this.labelAttr in item
							? item[this.labelAttr]
							: store.getLabel(item),
			...
			_fillContent: function () {
				...
				if (!this.options) {
					this.options = this.srcNodeRef
						? query('> *', this.srcNodeRef).map(function (node) {
								...
								return {
									value:
										node.getAttribute('data-' + kernel._scopeName + '-value') ||
			...
			postCreate: function () {
				...
				if (
					store &&
					(store.getIdentity || store.getFeatures()['dojo.data.api.Identity'])
				) {
			...
			_addOptionItem: function () /*__SelectOption*/
			/*===== option =====*/ {
				// summary:
			...
			_removeOptionItem: function () /*__SelectOption*/
			/*===== option =====*/ {
				// summary:
			...
			_setDisplay: function () /*String or String[]*/
			/*===== newDisplay =====*/ {
				// summary:
			...
			_pseudoLoadChildren: function () /*item[]*/
			/*===== items =====*/ {
				// summary:
