define([
	...
	=====*/
			...
			array.forEach(
				...
				function (child) {
					if ('value' in child) {
						...
						res = res.concat(
							this._getDescendantFormWidgets(child.getChildren()),
						)
					}
				},
				this,
		...
		validate: function () {
			...
			var didFocus = false
			return array.every(
				array.map(this._getDescendantFormWidgets(), function (widget) {
		...
		onValidStateChange: function () /*Boolean*/
		/*===== isValid =====*/ {
			// summary:
		...
		_getState: function () {
			...
			return array.indexOf(states, 'Error') >= 0
				? 'Error'
				: array.indexOf(states, 'Incomplete') >= 0
				? 'Incomplete'
				: ''
		},
