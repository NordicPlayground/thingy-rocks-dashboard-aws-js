define([
	...
	return declare(
		'dijit._MenuBase',
		[_Widget, _TemplatedMixin, _KeyNavContainer, _CssStateMixin],
		{
			// summary:
			...
			postCreate: function () {
				...
				this.own(
					on(
						this.containerNode,
						on.selector(matches, mouse.enter),
						function () {
					...
					on(
						this.containerNode,
						on.selector(matches, mouse.leave),
						function () {
			...
			onItemClick: function (/*dijit/_WidgetBase*/ item, /*Event*/ evt) {
				...
				if (item.popup) {
					...
					var byKeyboard =
						/^key/.test(evt._origType || evt.type) ||
			...
			_openItemPopup: function (
				...
				this.own(
					(this._mouseoverHandle = on.once(
						popup.domNode,
						'mouseover',
						lang.hitch(this, '_onPopupHover'),
					)),
				)
