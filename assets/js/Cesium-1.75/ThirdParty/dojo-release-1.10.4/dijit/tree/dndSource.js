define([
	...
	var dndSource = declare('dijit.tree.dndSource', _dndSelector, {
		...
		constructor: function (
			...
			var type =
				params.accept instanceof Array ? params.accept : ['text', 'treeNode']
			this.accept = null
			if (type.length) {
				this.accept = {}
		...
		_onDragMouse: function (e, firstTime) {
			...
			if (newTarget && this.betweenThreshold > 0) {
				...
				if (e.pageY - this.targetBox.y <= this.betweenThreshold) {
					...
					e.pageY - this.targetBox.y >=
					this.targetBox.h - this.betweenThreshold
				) {
			...
			if (
				firstTime ||
				newTarget != oldTarget ||
				newDropPosition != oldDropPosition
			) {
				if (oldTarget) {
				...
				if (!newTarget) {
					...
					m.canDrop(
						!dropOntoSelf &&
							...
							this.checkItemAcceptance(
								newTarget.rowNode,
								m.source,
								newDropPosition.toLowerCase(),
							),
					)
				}
				...
				if (
					...
					if (nodes.length) {
						...
						m.startDrag(
							this,
							nodes,
							this.copyState(dndCommon.getCopyKeyState(e)),
						)
		...
		onDndDrop: function (source, nodes, copy) {
			...
			if (this.containerState == 'Over') {
				...
				if (this.dropPosition == 'Before' || this.dropPosition == 'After') {
					...
					newParentItem =
						(target.getParent() && target.getParent().item) || tree.item
						...
						if (source == this) {
							...
							if (typeof insertIndex == 'number') {
								if (
									...
									childTreeNode.getIndexInParent() < insertIndex
								) {
									insertIndex -= 1
								}
							}
