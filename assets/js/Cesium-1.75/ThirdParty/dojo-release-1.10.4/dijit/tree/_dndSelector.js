define([
	...
	return declare('dijit.tree._dndSelector', _dndContainer, {
		...
		constructor: function () {
			...
			this.events.push(
				...
				on(
					this.tree.domNode,
					a11yclick.release,
					lang.hitch(this, 'onClickRelease'),
				),
			)
		},
		...
		setSelection: function (/*dijit/Tree._TreeNode[]*/ newSelection) {
			...
			array.forEach(
				this._setDifference(oldSelection, newSelection),
			...
			array.forEach(
				this._setDifference(newSelection, oldSelection),
				lang.hitch(this, function (node) {
		...
		onClickRelease: function (e) {
			...
			this._doDeselect = false
			this.userSelect(
				e.type == 'keyup' ? this.tree.focusedChild : this.current,
				dndCommon.getCopyKeyState(e),
				e.shiftKey,
			)
		},
