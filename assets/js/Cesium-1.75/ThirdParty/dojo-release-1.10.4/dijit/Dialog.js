define([
	...
	var _DialogBase = declare(
		...
		[_TemplatedMixin, _FormMixin, _DialogMixin, _CssStateMixin],
		{
			...
			_endDrag: function () {
				...
				nodePosition.y = Math.min(
					Math.max(nodePosition.y, 0),
					viewport.h - nodePosition.h,
				)
				nodePosition.x = Math.min(
					Math.max(nodePosition.x, 0),
					viewport.w - nodePosition.w,
				)
			...
			_setup: function () {
				...
				if (this.titleBar && this.draggable) {
					this._moveable = new (
						has('ie') == 6
							? TimedMoveable // prevent overload, see #5285
							: Moveable
					)(node, { handle: this.titleBar })
					aspect.after(
						this._moveable,
						'onMoveStop',
			...
			show: function () {
				...
				this._modalconnects.push(
					on(win, 'scroll', lang.hitch(this, 'resize', null)),
				)
			...
			resize: function (dim) {
				...
				if (this.domNode.style.display != 'none') {
					...
					if (dim) {
						...
						if (this._singleChild) {
							var cb = utils.marginBox2contentBox(
								this.containerNode,
								centerSize,
							)
	...
	var DialogLevelManager = (Dialog._DialogLevelManager = {
		...
		show: function (/*dijit/_WidgetBase*/ dialog, /*Object*/ underlayAttrs) {
			...
			var zIndex = ds[ds.length - 1].dialog
				? ds[ds.length - 1].zIndex + 2
		...
		hide: function (/*dijit/_WidgetBase*/ dialog) {
			...
			if (ds[ds.length - 1].dialog == dialog) {
				...
				if (dialog.refocus) {
					...
					if (
						pd.dialog &&
						(!focus || !dom.isDescendant(focus, pd.dialog.domNode))
					) {
	...
	focus.watch('curNode', function (attr, oldNode, node) {
		...
		if (
			node &&
			topDialog &&
			!topDialog._fadeOutDeferred &&
			node.ownerDocument == topDialog.ownerDocument
		) {
			...
			do {
				if (
					node == topDialog.domNode ||
					domClass.contains(node, 'dijitPopup')
				) {
					return
				}
