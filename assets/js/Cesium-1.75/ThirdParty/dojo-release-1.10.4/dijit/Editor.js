define([
	...
	var Editor = declare('dijit.Editor', RichText, {
		...
		postCreate: function () {
			...
			this.commands = i18n.getLocalization(
				'dijit._editor',
				'commands',
				this.lang,
			)
		...
		addPlugin: function (
			...
			var args = lang.isString(plugin)
				? { name: plugin }
				: lang.isFunction(plugin)
				? { ctor: plugin }
				: plugin
			if (!args.setEditor) {
				...
				if (!o.plugin) {
					try {
						...
						var pc =
							args.ctor || lang.getObject(args.name) || require(args.name)
						if (pc) {
							o.plugin = new pc(args)
						}
					} catch (e) {
						throw new Error(
							this.id + ': cannot find plugin [' + args.name + ']',
						)
					}
				}
				if (!o.plugin) {
		...
		_clipboardCommand: function (cmd) {
			...
			try {
				...
				if (
					e.code == 1011 /* Mozilla: service denied */ ||
					...
					alert(
						sub(this.commands.systemShortcut, [
							this.commands[cmd],
							sub(this.commands[has('mac') ? 'appleKey' : 'ctrlKey'], [
								accel[cmd],
							]),
						]),
					)
						...
						b.mark = {
							...
							endContainer:
								b.mark.endContainer === b.mark.startContainer
									? tmp
									: rangeapi.getIndex(b.mark.endContainer, this.editNode).o,
			...
			switch (k) {
				...
				default:
					if (
						...
						!e.altKey &&
						!e.metaKey &&
						(e.keyCode < keys.F1 || e.keyCode > keys.F15)
					) {
						this.beginEditing()
						break
					}
			...
			try {
				...
				if (this.document && this.document.body) {
					domStyle.set(
						this.document.body,
						'color',
						domStyle.get(this.iframe, 'color'),
					)
				}
			} catch (e) {
