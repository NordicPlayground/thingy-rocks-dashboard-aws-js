define([
	...
	var SplitContainer = declare('dijit.layout.SplitContainer', _LayoutWidget, {
		...
		postCreate: function () {
			...
			sizer.className = this.isHorizontal
		...
		_checkSizes: function () {
			...
			if (totalMinSize <= totalSize) {
				...
				array.forEach(children, function (child) {
					child.sizeActual = Math.round(
						totalSize * (child.sizeMin / totalMinSize),
					)
				})
			}
		},
		...
		beginSizing: function (e, i) {
			...
			this.sizingSplitter.positionBeforeDrag = domStyle.get(
				this.sizingSplitter,
				this.isHorizontal ? 'left' : 'top',
			)

			if (!this.cover) {
				this.cover = domConstruct.create(
					'div',
					{
						style: {
							position: 'absolute',
			...
			this.minDelta =
				-1 * (this.paneBefore.sizeActual - this.paneBefore.sizeMin)
			...
			this._ownconnects = [
				on(
					...
					'mousemove',
					lang.hitch(this, 'changeSizing'),
				),
				on(
					this.ownerDocument.documentElement,
					'mouseup',
					lang.hitch(this, 'endSizing'),
				),
			]
		...
		changeSizing: function (e) {
			...
			var delta = Math.max(
				Math.min(this.lastPoint - this.startPoint, this.maxDelta),
				this.minDelta,
			)
