	...
	var _Splitter = declare(
		...
		[_Widget, _TemplatedMixin],
		{
		 ...
		 =====*/
			...
			templateString:
				'<div class="dijitSplitter" data-dojo-attach-event="onkeydown:_onKeyDown,press:_startDrag,onmouseenter:_onMouse,onmouseleave:_onMouse" tabIndex="0" role="separator"><div class="dijitSplitterThumb"></div></div>',
			...
			_startDrag: function (e) {
				...
				var factor = this._factor,
					...
					childStart = domGeometry.getMarginBox(this.child.domNode, childCS)[
						dim
					],
					max = this._computeMaxSize(),
					min = Math.max(
						this.child.minSize,
						domGeometry.getPadBorderExtents(this.child.domNode, childCS)[dim] +
							10,
					),
					region = this.region,
				...
				this._handlers = this._handlers.concat([
					on(
						de,
						touch.move,
						(this._drag = function (e, forceResize) {
							...
							splitterStyle[splitterAttr] =
								delta +
								splitterStart +
								factor * (boundChildSize - childSize) +
								'px'
						}),
					),
			...
			_onKeyDown: function (/*Event*/ e) {
				...
				var childSize =
					domGeometry.getMarginSize(this.child.domNode)[
						horizontal ? 'h' : 'w'
					] +
					this._factor * tick
		...
		LayoutContainer,
		{
			...
			_setupChild: function (/*dijit/_WidgetBase*/ child) {
				...
				if (region) {
					...
					if (
						region != 'center' &&
						(child.splitter || this.gutters) &&
						!child._splitterWidget
					) {
						...
						var before =
							region == 'bottom' ||
							region == (this.isLeftToRight() ? 'right' : 'left')
						domConstruct.place(
							splitter.domNode,
							child.domNode,
							before ? 'before' : 'after',
