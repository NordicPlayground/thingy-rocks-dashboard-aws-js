define([
	...
	var ScrollingTabController = declare(
		'dijit.layout.ScrollingTabController',
		[TabController, _WidgetsInTemplateMixin],
		{
			...
			_initButtons: function () {
				...
				this._btnWidth = 0
				this._buttons = query('> .tabStripButton', this.domNode).filter(
					function (btn) {
						if (
			...
			_getTabsWidth: function () {
				...
				if (children.length) {
					var leftTab =
							children[this.isLeftToRight() ? 0 : children.length - 1].domNode,
						rightTab =
							children[this.isLeftToRight() ? children.length - 1 : 0].domNode
			...
			resize: function (dim) {
				...
				var cb = (this._contentBox = layoutUtils.marginBox2contentBox(
					this.domNode,
					{ h: 0, w: dim.w },
				))
				...
				if (this._selectedTab) {
					...
					this.scrollNode.scrollLeft = this._convertToScrollLeft(
			...
			_getScroll: function () {
				...
				return this.isLeftToRight() ||
					...
					: domStyle.get(this.containerNode, 'width') -
							domStyle.get(this.scrollNode, 'width') +
				...
				if (children.length && tabsWidth > scrollNodeWidth) {
					...
					return {
						min: this.isLeftToRight()
							? 0
							: children[children.length - 1].domNode.offsetLeft,
						max: this.isLeftToRight()
							? children[children.length - 1].domNode.offsetLeft +
							  children[children.length - 1].domNode.offsetWidth -
							  scrollNodeWidth
							: maxPossibleScroll,
					}
				} else {
			...
			_getScrollForSelectedTab: function () {
				...
				var pos =
					n.offsetLeft + domStyle.get(n, 'width') / 2 - scrollNodeWidth / 2
	...
	var ScrollingTabControllerButtonMixin = declare(
