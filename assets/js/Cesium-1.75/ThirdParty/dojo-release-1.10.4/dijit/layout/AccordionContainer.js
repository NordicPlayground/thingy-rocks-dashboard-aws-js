define([
	...
	var AccordionButton = declare(
		'dijit.layout._AccordionButton',
		[_Widget, _TemplatedMixin, _CssStateMixin],
		{
			// summary:
			...
			buildRendering: function () {
				...
				this.focusNode.setAttribute(
					'aria-labelledby',
					domAttr.get(this.titleTextNode, 'id'),
				)
	...
	var AccordionInnerContainer = declare(
		'dijit.layout._AccordionInnerContainer' +
			(has('dojo-bidi') ? '_NoBidi' : ''),
		[_Widget, _CssStateMixin],
		{
			// summary:
		 ...
		 =====*/
			...
			buildRendering: function () {
				...
				this.domNode = domConstruct.place(
					"<div class='" + this.baseClass + "' role='presentation'>",
				...
				var child = this.contentWidget,
					cls = lang.isString(this.buttonWidget)
				...
				this.containerNode = domConstruct.place(
	...
	if (has('dojo-bidi')) {
		AccordionInnerContainer = declare(
			'dijit.layout._AccordionInnerContainer',
	...
	var AccordionContainer = declare(
		'dijit.layout.AccordionContainer',
		StackContainer,
		{
			// summary:
		 ...
		 =====*/
			...
			layout: function () {
				...
				var wrapperDomNode = openPane._wrapperWidget.domNode,
					...
					wrapperDomNodePadBorder =
						domGeometry.getPadBorderExtents(wrapperDomNode),
					wrapperContainerNode = openPane._wrapperWidget.containerNode,
					wrapperContainerNodeMargin =
						domGeometry.getMarginExtents(wrapperContainerNode),
					wrapperContainerNodePadBorder =
						domGeometry.getPadBorderExtents(wrapperContainerNode),
				...
				array.forEach(this.getChildren(), function (child) {
					if (child != openPane) {
						...
						totalCollapsedHeight += domGeometry.getMarginSize(
							child._wrapperWidget.domNode,
						).h
					}
				})
				this._verticalSpace =
			...
			_transition: function (
				/*dijit/_WidgetBase?*/ newWidget,
				/*dijit/_WidgetBase?*/ oldWidget,
				/*Boolean*/ animate,
			) {
				...
				if (animate) {
					...
					var wrapperContainerNode = newWidget._wrapperWidget.containerNode,
						...
						animationHeightOverhead =
							wrapperContainerNodeMargin.h + wrapperContainerNodePadBorder.h

					oldContents.style.height =
			...
			_onKeyDown: function (/*Event*/ e, /*dijit/_WidgetBase*/ fromTitle) {
				...
				var c = e.keyCode
				if (
					(fromTitle && (c == keys.LEFT_ARROW || c == keys.UP_ARROW)) ||
					...
					(fromTitle && (c == keys.RIGHT_ARROW || c == keys.DOWN_ARROW)) ||
