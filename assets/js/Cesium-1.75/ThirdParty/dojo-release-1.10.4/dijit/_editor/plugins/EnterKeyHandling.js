	...
	return declare('dijit._editor.plugins.EnterKeyHandling', _Plugin, {
			...
			if (this._checkListLater) {
				if (this.editor.selection.isCollapsed()) {
					...
					if (!liparent) {
						...
						RichText.prototype.execCommand.call(
							this.editor,
							'formatblock',
							this.blockNodeForEnter,
						)
						// set the innerHTML of the new block node
						var block = this.editor.selection.getAncestorElement(
							this.blockNodeForEnter,
						)
						if (block) {
		...
		handleEnterKey: function (e) {
			...
			if (e.shiftKey) {
				...
				if (header) {
					...
					if (
						rangeapi.atBeginningOfContainer(
							header,
						...
						if (rs && rs.nodeType == 3) {
							...
							startNode = doc.createTextNode(
								txt.substring(0, range.startOffset),
							)
			...
			var block = rangeapi.getBlockAncestor(
				range.endContainer,
				null,
			...
			if (
				(this._checkListLater =
					blockNode &&
					(blockNode.nodeName == 'LI' || blockNode.parentNode.nodeName == 'LI'))
			) {
				if (has('mozilla')) {
			...
			if (!block.blockNode || block.blockNode === this.editor.editNode) {
				...
				if (block.blockNode) {
					if (
						...
						!(block.blockNode.textContent || block.blockNode.innerHTML).replace(
							/^\s+|\s+$/g,
							'',
						).length
					) {
		...
		removeTrailingBr: function (container) {
			...
			var para = /P|DIV|LI/i.test(container.tagName)
				? container
				: this.editor.selection.getParentOfType(container, ['P', 'DIV', 'LI'])
