define([
	...
	var ViewSource = declare('dijit._editor.plugins.ViewSource', _Plugin, {
		...
		_initButton: function () {
			...
			if (has('ie') == 7) {
				this._ieFixNode = domConstruct.create(
					'div',
					{
						style: {
							opacity: '0',
		...
		_showSource: function (source) {
			...
			try {
				...
				if (source) {
					...
					this._setListener = aspect.after(
						this.editor,
						'setValue',
						lang.hitch(this, function (htmlTxt) {
							htmlTxt = htmlTxt || ''
		...
		_resize: function () {
			...
			var containerPadding = domGeometry.getPadBorderExtents(
				ed.iframe.parentNode,
			)
	...
	_Plugin.registry['viewSource'] = _Plugin.registry['viewsource'] = function (
		args,
	) {
