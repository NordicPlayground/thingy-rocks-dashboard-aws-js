define([
	...
	var FullScreen = declare('dijit._editor.plugins.FullScreen', _Plugin, {
		...
		setEditor: function (editor) {
			...
			this.own(
				on(this.editor.domNode, 'keydown', lang.hitch(this, '_containFocus')),
			)
		},
		...
		_resizeEditor: function () {
			...
			var fcpExtents = domGeometry.getPadBorderExtents(
				this.editor.iframe.parentNode,
			)
			var fcmExtents = domGeometry.getMarginExtents(
				this.editor.iframe.parentNode,
			)
			...
			if (full) {
				...
				if (has('ie') && !has('quirks')) {
					...
					if (
						...
						try {
							this._oldBodyParentOverflow = domStyle.get(
								body.parentNode,
								'overflow',
							)
						} catch (e) {
				...
				this._resizeHandle2 = aspect.after(
					ed,
					'onResize',
					lang.hitch(this, function () {
						if (this._resizer) {
	...
	_Plugin.registry['fullScreen'] = _Plugin.registry['fullscreen'] = function (
		args,
	) {
