define([
	...
	var rangeapi = {
		...
		getBlockAncestor: function (
			...
			return {
				blockNode: block,
				blockContainer: blockContainer || node.ownerDocument.body,
			}
		},

		atBeginningOfContainer: function (
			/*DomNode*/ container,
			/*DomNode*/ node,
			/*Int*/ offset,
		) {
	...
	if (!window.getSelection) {
		var ie = (rangeapi.ie = {
			...
			decomposeControlRange: function (range) {
				var firstnode = range.item(0),
					lastnode = range.item(range.length - 1)
				var startContainer = firstnode.parentNode,
						...
						if (node) {
							if (node.nodeType == 3) {
								...
								if (node.nextSibling && node.nextSibling.nodeType == 3) {
									...
									atmrange.moveToElementText(
										node.nextSibling ? node : container,
									)
									var parent = node.parentNode
									var tempNode = parent.insertBefore(
			...
			decomposeTextRange: function (range) {
				...
				var startContainer = tmpary[0],
					startOffset = tmpary[1]
				var endContainer = tmpary[0],
					endOffset = tmpary[1]
		...
		var W3CRange = (rangeapi.W3CRange = declare(null, {
			...
			_updateInternal: function () {
				if (this.startContainer !== this.endContainer) {
					this.commonAncestorContainer = rangeapi.getCommonAncestor(
						this.startContainer,
						this.endContainer,
					)
				} else {
