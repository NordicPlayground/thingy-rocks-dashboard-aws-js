	...
	var RichText = declare('dijit._editor.RichText', [_Widget, _CssStateMixin], {
		...
		postCreate: function () {
			...
			if (has('ie') || has('trident')) {
				...
				this.contentDomPostFilters = [
					lang.hitch(this, '_stripBreakerNodes'),
				].concat(this.contentDomPostFilters)
			}
			this.contentDomPostFilters = [
				lang.hitch(this, '_stripTrailingEmptyNodes'),
			].concat(this.contentDomPostFilters)
		...
		_localizeEditorCommands: function () {
			...
			while ((format = formats[i++])) {
				...
				if (format.charAt(1) !== 'l') {
					localhtml +=
						'<' + format + '><span>content</span></' + format + '><br/>'
				} else {
			...
			var inject = lang.hitch(this, function () {
				...
				while (node) {
					try {
						...
						this._local2NativeFormatNames[nativename] =
		...
		open: function (/*DomNode?*/ element) {
			...
			if (
				this.name !== '' &&
				(!config['useXDomain'] || config['allowXdRichTextSave'])
			) {
				var saveTextarea = dom.byId(
					dijit._scopeName + '._editor.RichText.value',
				)
		...
		_getIframeDocTxt: function () {
			...
			var userStyle = ''
			var self = this
			this.style.replace(/(^|;)\s*(line-|font-?)[^;]+/gi, function (match) {
		...
		_applyEditingAreaStyleSheets: function () {
			...
			var text = '',
				i = 0,
				url,
				ownerWindow = winUtils.get(this.ownerDocument)
			while ((url = files[i++])) {
		...
		onLoad: function (/*String*/ html) {
			...
			if (has('ie')) {
				// IE contentEditable
				this.own(
					on(this.document, 'mousedown', lang.hitch(this, '_onIEMouseDown')),
				) // #4996 fix focus
			...
			if (has('webkit')) {
				...
				this._webkitListener = this.own(
					on(this.document, 'mouseup', lang.hitch(this, 'onDisplayChanged')),
				)[0]
				this.own(
					on(
						this.document,
		...
		_queryCommandAvailable: function (/*String*/ command) {
			...
			return (
				((has('ie') || has('trident')) && supportedBy.ie) ||
		...
		_removeMozBogus: function (/* String */ html) {
			...
			return html
				...
				.replace(/_moz_resizing="(true|false)"/gi, '') // String
		},
		...
		_browserQueryCommandEnabled: function (command) {
			...
			var elem =
				has('ie') < 9 ? this.document.selection.createRange() : this.document
			try {
		...
		_hilitecolorImpl: function (argument) {
			...
			if (!isApplied) {
				if (has('mozilla')) {
					...
					returnValue = this.document.execCommand(
						'hilitecolor',
						false,
						argument,
					)
		...
		_insertorderedlistImpl: function (argument) {
			...
			if (!applied) {
				applied = this.document.execCommand(
					'insertorderedlist',
					false,
					argument,
				)
		...
		_adaptIESelection: function () {
			...
			if (selection && selection.rangeCount && !selection.isCollapsed) {
				...
				while (
					firstNode.nodeType === 3 /*text*/ &&
					startOffset >= firstNode.length &&
				...
				var lastNode = null
				while (
					this._isNodeEmpty(firstNode, startOffset) &&
					firstNode !== lastNode
				) {
					...
					if (topNode) {
						...
						if (rs && rs.nodeType === 3) {
							...
							if (endNode) {
								nodeToMove = endNode
								if (
									nodeToMove.nodeType === 1 ||
									(nodeToMove.nodeType === 3 && nodeToMove.nodeValue)
								) {
