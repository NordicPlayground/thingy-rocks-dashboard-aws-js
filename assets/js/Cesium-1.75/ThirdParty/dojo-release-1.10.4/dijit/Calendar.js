define([
	...
	var Calendar = declare(
		'dijit.Calendar',
		[CalendarLite, _Widget, _CssStateMixin],
		{
			// summary:
			...
			postCreate: function () {
				...
				this.own(
					...
					on(
						this.dateRowsNode,
						'mouseover',
						lang.hitch(this, '_onDayMouseOver'),
					),
					on(this.dateRowsNode, 'mouseout', lang.hitch(this, '_onDayMouseOut')),
					on(
						this.dateRowsNode,
						'mousedown',
						lang.hitch(this, '_onDayMouseDown'),
					),
			...
			_onDayMouseOver: function (/*Event*/ evt) {
				...
				// set node to the <td>.
				var node = domClass.contains(evt.target, 'dijitCalendarDateLabel')
				...
				if (
					node &&
					((node.dijitDateValue &&
						!domClass.contains(node, 'dijitCalendarDisabledDate')) ||
	...
	Calendar._MonthDropDownButton = declare(
	...
	Calendar._MonthDropDown = declare(
		...
		[_Widget, _TemplatedMixin, _CssStateMixin],
		{
			...
			templateString:
				"<div data-dojo-attach-event='ondijitclick:_onClick'></div>",
