define([
	...
	return declare('dijit.Menu', DropDownMenu, {
		...
		_iframeContentWindow: function (/* HTMLIFrameElement */ iframe_el) {
			...
			//		private
			return (
				winUtils.get(this._iframeContentDocument(iframe_el)) ||
		...
		_iframeContentDocument: function (/* HTMLIFrameElement */ iframe_el) {
			...
			//		protected
			return (
				iframe_el.contentDocument || // W3
				(iframe_el.contentWindow && iframe_el.contentWindow.document) || // IE
				(iframe_el.name &&
					document.frames[iframe_el.name] &&
		...
		bindDomNode: function (/*String|DomNode*/ node) {
			...
			if (node.tagName.toLowerCase() == 'iframe') {
				...
				cn =
					node == win.body(this.ownerDocument)
			...
			var doConnects = lang.hitch(this, function (cn) {
				...
				return [
					on(
						cn,
						delegatedEvent(this.leftClickToOpen ? 'click' : 'contextmenu'),
						function (evt) {
							evt.stopPropagation()
							evt.preventDefault()
					...
					on(cn, delegatedEvent('keydown'), function (evt) {
						if (
							evt.keyCode == 93 || // context menu key
		...
		_openMyself: function (args) {
			...
			if (coords) {
				if (iframe) {
					...
					var cs = domStyle.getComputedStyle(iframe),
						...
						left =
							(has('ie') && has('quirks') ? 0 : tp(iframe, cs.paddingLeft)) +
							(has('ie') && has('quirks') ? tp(iframe, cs.borderLeftWidth) : 0),
						top =
							(has('ie') && has('quirks') ? 0 : tp(iframe, cs.paddingTop)) +
							(has('ie') && has('quirks') ? tp(iframe, cs.borderTopWidth) : 0)
			...
			var savedFocusNode =
				!curFocusNode || dom.isDescendant(curFocusNode, this.domNode)
					? prevFocusNode
					: curFocusNode
