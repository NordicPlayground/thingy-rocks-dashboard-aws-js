define([
	...
	var TreeNode = declare(
		...
		[_Widget, _TemplatedMixin, _Container, _Contained, _CssStateMixin],
		{
			...
			_applyClassAndStyle: function (item, lower, upper) {
				...
				this[clsName] = this.tree['get' + upper + 'Class'](
					item,
					this.isExpanded,
				)
				...
				domStyle.set(
					this[nodeName],
					this.tree['get' + upper + 'Style'](item, this.isExpanded) || {},
				)
			},
			...
			_updateLayout: function () {
				...
				var parent = this.getParent(),
					markAsRoot =
						!parent || !parent.rowNode || parent.rowNode.style.display == 'none'
				domClass.toggle(this.domNode, 'dijitTreeIsRoot', markAsRoot)

				domClass.toggle(
					this.domNode,
					'dijitTreeIsLast',
					!markAsRoot && !this.getNextSibling(),
				)
			},
				...
				this.defer(function () {
					array.forEach(oldChildren, function (node) {
						if (!node._destroyed && !node.getParent()) {
							...
							if (tree.persist) {
								var destroyedPath = array
									.map(node.getTreePath(), function (item) {
							...
							if (
								...
								!dom.isDescendant(tree.lastFocusedChild, tree.domNode)
							) {
								delete tree.lastFocusedChild
							}
							if (
								focusedChild &&
								!dom.isDescendant(focusedChild, tree.domNode)
							) {
	...
	var Tree = declare(
		'dijit.Tree',
		[_Widget, _KeyNavMixin, _TemplatedMixin, _CssStateMixin],
		{
			// summary:
		...
		=====*/
			...
			_publish: function (/*String*/ topicName, /*Object*/ message) {
				...
				topic.publish(
					this.id,
					lang.mixin({ tree: this, event: topicName }, message || {}),
				) // publish
			},
			...
			postCreate: function () {
				...
				this.own(
					on(
						this.containerNode,
						on.selector('.dijitTreeNode', touch.enter),
						function (evt) {
					...
					on(
						this.containerNode,
						on.selector('.dijitTreeNode', touch.leave),
						function (evt) {
					...
					on(
						this.containerNode,
						on.selector('.dijitTreeRow', a11yclick.press),
						function (evt) {
					...
					on(
						this.containerNode,
						on.selector('.dijitTreeRow', a11yclick),
						function (evt) {
			...
			_store2model: function () {
				...
				if (this.params.getItemChildren) {
					modelParams.getChildren = lang.hitch(
						this,
						function (item, onComplete, onError) {
					...
					this.pendingCommandsPromise.always(
						lang.hitch(this, function () {
							var identities = array.map(items, function (item) {
								return !item || lang.isString(item)
									? item
			...
			_setPathAttr: function (/*Item[]|String[]*/ path) {
				...
				if (path.length) {
					return shimmedPromise(
						this.set('paths', [path]).then(function (paths) {
							return paths[0]
						}),
					)
				} else {
					...
					return shimmedPromise(
						this.set('paths', []).then(function (paths) {
							return paths[0]
						}),
					)
				}
			...
			expandAll: function () {
				...
				function expand(node) {
					...
					return _this._expandNode(node).then(function () {
						...
						var childBranches = array.filter(
							node.getChildren() || [],
							function (node) {
			...
			mayHaveChildren: function () /*dojo/data/Item*/
			/*===== item =====*/ {
				// summary:
			...
			getTooltip: function () /*dojo/data/Item*/
			/*===== item =====*/ {
				// summary:
			...
			_onLeftArrow: function (/*Event*/ evt, /*TreeNode*/ node) {
				...
				if (node.isExpandable && node.isExpanded) {
					...
					if (
						parent &&
						parent.isTreeNode &&
						!(!this.showRoot && parent === this.rootNode)
					) {
			...
			isExpandoNode: function (node, widget) {
				...
				return (
					dom.isDescendant(node, widget.expandoNode) ||
					dom.isDescendant(node, widget.expandoNodeText)
				)
			},
			...
			_onNodeMouseEnter: function () /*dijit/_WidgetBase*/
			/*===== node =====*/ {
				// summary:
			...
			_onNodeMouseLeave: function () /*dijit/_WidgetBase*/
			/*===== node =====*/ {
				// summary:
					...
					array.forEach(
						...
						function (node) {
							...
							if (
								...
								!dom.isDescendant(this.lastFocusedChild, this.domNode)
							) {
								delete this.lastFocusedChild
							}
							if (
								this.focusedChild &&
								!dom.isDescendant(this.focusedChild, this.domNode)
							) {
								this.focus()
							}
			...
			resize: function (changeSize) {
				...
				this._nodePixelIndent =
					domGeometry.position(this.tree.indentDetector).w ||
					this._nodePixelIndent
