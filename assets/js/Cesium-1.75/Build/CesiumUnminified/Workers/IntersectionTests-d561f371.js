...
define([
	...
	QuadraticRealPolynomial.computeRealRoots = function (a, b, c) {
		...
		var b2 = b * b
		var four_ac = 4.0 * a * c
		var radicand = addWithCancellationCheck(
			b2,
			-four_ac,
	...
	IntersectionTests.rayPlane = function (ray, plane, result) {
		...
		var t =
			(-plane.distance - Cartesian2.Cartesian3.dot(normal, origin)) /
			denominator
	...
	IntersectionTests.lineSegmentTriangle = function (
		...
		if (
			!when.defined(t) ||
			t < 0.0 ||
			t > Cartesian2.Cartesian3.distance(v0, v1)
		) {
			return undefined
		}
	...
	IntersectionTests.rayEllipsoid = function (ray, ellipsoid) {
		...
		var q = Cartesian2.Cartesian3.multiplyComponents(
			inverseRadii,
			ray.origin,
			scratchQ,
		)
		var w = Cartesian2.Cartesian3.multiplyComponents(
			inverseRadii,
			ray.direction,
			scratchW,
		)
	...
	function quadraticVectorExpression(A, b, c, x, w) {
		...
		var l2 =
			(A[Transforms.Matrix3.COLUMN1ROW1] - A[Transforms.Matrix3.COLUMN2ROW2]) *
			wSquared
		var l1 =
			w *
			(x *
		...
		for (var i = 0; i < length; ++i) {
			...
			if (_Math.CesiumMath.sign(l2) === _Math.CesiumMath.sign(l0)) {
				...
				_Math.CesiumMath.sign(l0) === _Math.CesiumMath.sign(l1 * cosine)
			) {
				left = addWithCancellationCheck$1(
					l2 * cosineSquared,
					l1 * cosine + l0,
			...
			var right = addWithCancellationCheck$1(
				r1 * cosine,
				r0,
				_Math.CesiumMath.EPSILON15,
			)
	...
	IntersectionTests.grazingAltitudeLocation = function (ray, ellipsoid) {
		...
		var f = ellipsoid.transformPositionToScaledSpace(
			direction,
			firstAxisScratch,
		)
		...
		var reference = Cartesian2.Cartesian3.mostOrthogonalAxis(
			f,
			referenceScratch,
		)
		...
		if (length > 0) {
			var closest = Cartesian2.Cartesian3.clone(
