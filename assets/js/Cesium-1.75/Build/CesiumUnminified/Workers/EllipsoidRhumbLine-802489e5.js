...
define([
	...
	function computeProperties(ellipsoidRhumbLine, start, end, ellipsoid) {
		...
		Check.Check.typeOf.number.greaterThanOrEquals(
			...
			Math.abs(
				Math.abs(
					Cartesian2.Cartesian3.angleBetween(firstCartesian, lastCartesian),
				) - Math.PI,
			),
			0.0125,
		)
		...
		ellipsoidRhumbLine._end = Cartesian2.Cartographic.clone(
			end,
	...
	function interpolateUsingSurfaceDistance(
		...
		if (
			Math.abs(_Math.CesiumMath.PI_OVER_TWO - Math.abs(heading)) >
			_Math.CesiumMath.EPSILON8
		) {
			...
			longitude = _Math.CesiumMath.negativePiToPi(
	...
	EllipsoidRhumbLine.prototype.findIntersectionWithLongitude = function (
		...
		intersectionLongitude = _Math.CesiumMath.negativePiToPi(
			intersectionLongitude,
		)
		...
		do {
			...
			!_Math.CesiumMath.equalsEpsilon(newPhi, phi, _Math.CesiumMath.EPSILON12)
		)
	...
	EllipsoidRhumbLine.prototype.findIntersectionWithLatitude = function (
		...
		var longitude = _Math.CesiumMath.negativePiToPi(
			start.longitude + deltaLongitude,
		)
